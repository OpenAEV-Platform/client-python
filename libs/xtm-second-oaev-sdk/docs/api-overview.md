...
`xtm-second-oaev-sdk` organises its implementation across three explicit layers. The root `xtm_second_oaev_sdk/__init__.py` re-exports exactly 107 stable symbols via `public/` and is the only surface consumers should target.
...
```python
from xtm_second_oaev_sdk import SecurityDomains
