...
`xtm-oaev-sdk` exposes the same 107 public symbols as `xtm-oaev-sdk` but organises its internals into three explicit layers: stable data-model contracts (`contracts/`), feature implementations (`_core/`), and a flat re-export surface (`public/`). The public API is identical; only the internal layout changes.
...
from xtm_oaev_sdk._core.signatures import SignatureManager
```
...
- [Migration Guide](docs/migration-guide.md) — moving from `xtm-oaev-sdk` to `xtm-oaev-sdk`
...
python -c "from xtm_oaev_sdk import __all__; print(len(__all__))"
```
